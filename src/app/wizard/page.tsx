...
import { DEMO_MODELS, blendedPrice } from "@/lib/demo/models";
...
/** 按答案微调评分后排序,返回带分值的模型列表(高到低)。 */
function rankModels(answers: Record<string, string>) {
...
function buildRecommendation(model: Model, answers: Record<string, string>): Recommendation {
  const sceneLabel =
...
export default function WizardPage() {
  ...
  const ranked = done ? rankModels(answers) : [];
  const top = ranked[0];
